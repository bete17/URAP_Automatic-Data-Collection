    ...
    for row_index in range(5): #range(end_index):
        ...
        if item7_key not in completed7:
            try:
                ...
                gpt7.getContent(7)
                txt7, tokens, time, prompt_length = gpt7.push()
                log7.record(tokens, time, prompt_length)
        ...
        if item8_key not in completed8:
            try:
                ...
                gpt8.getContent(8)
                txt8, tokens, time, prompt_length = gpt8.push()
                log8.record(tokens, time, prompt_length)
